...
class dataIter(object):
	def __init__(self, cfg=None):
		...
		self.data = None
		self.label = None
		dataIter.read_vos_train_list(self,cfg)
	...
	def get_batch(cfg, data_list, label_list, indexes):
		...
		build databatch
		"""
		Sizes = [[352,352],[416,416],[448,448],[480,480],[512,512],[640,640]]
	...
	def read_vos_train_list(self,cfg):
		...
		with open(path_to_file) as f:
			...
			#f =['train ']
			for line in f:
				for filename in os.listdir(os.path.join(path_to_img,line[:-1])):
					name, extension = os.path.splitext(filename)
		
					self.data_list.append(os.path.join(path_to_img,line[:-1],name+'.jpg'))
					self.label_list.append(os.path.join(path_to_gt,line[:-1],name+'.png'))		
